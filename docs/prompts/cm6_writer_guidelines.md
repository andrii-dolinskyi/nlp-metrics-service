...
- Use the core keyword from the page brief naturally in the first 60 words of the page
- Use it in one H2 only when a supplied heading already contains it. Never change a heading to fit it in.
- Outside headings, use the core keyword at most three more times across the page, and use the secondary keywords only where they read naturally. There is no count to hit: a forced keyword is a failure, a missing keyword is not.
