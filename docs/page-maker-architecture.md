...
| Validate Draft | outline order and verbatim headings, link presence and placement, fact discipline, citations and tables from the spec, constraints, style; no density. Removed on 2026-09-21: the page goes from Extract Updated straight to the assets |
...
| DeepL Translate | inputs to English before the writer; page, meta description, FAQ and E-E-A-T back to the target language in one batch, links preserved, app headings restored |
...
Revision of 2026-09-15 (after the first production review):

- **No internal linking.** `internalLinks` is ignored in the request, the writer is told never to link to the client's site, and the validator rejects any link to `siteRootUrl` other than the CTA link. A separate automation places internal links after the page is written.
- **Closing section per page type.** Eight spec columns were added (`closing_mode`, `closing_heading`, `closing_content`, `cta_mode`, `cta_guidance`, `format_rules`, `h3_policy`, `meta_description_pattern`; values for all 145 types in `docs/page-type-writing-rules.md`, generated by `tools/extend_specs.py`). When `closing_mode` is `append`, Page Contract adds one closing H2 after the app's outline unless the last outline heading already reads as a closing section; the validator expects outline plus one H2 and rejects generic closing headings (Conclusion, Summary, Final thoughts).
- **CTA in the closing section only.** `cta_mode` per type (required for offer, proof, entity, local and most catalogue types; optional for guides, evaluations, tools and assets; none for reference, ops, hubs without a service). The validator flags the CTA link or CTA language anywhere else. The FAQ never names the client and never asks for an action; Collect FAQ strips such sentences if the model slips.
- **Formats and H3s from the spec.** The request no longer carries per-heading formats; `format_rules` tells the writer where this type uses tables, steps, bullets and prose, and `h3_policy` (required, optional, none) is validated: at least two H3s per section when used, none in sections under 150 words, none on types that forbid them.
- **Smaller input.** Required fields are `pageType`, `clientName`, `callback_url`, `h1` and `h2Outline`; `slugPath` is derived from the H1 when empty. Facts stay required for the families that make claims about the client.
...
- **Requests arrive in the target language (2026-09-22).** The app stores every text value in the page language, so a non-English request is translated to English before Parse Request: `Translate Inputs?` (on `targetLanguage`), `DeepL Prep Inputs` (lists `clientDescription`, `h1`, `coreKeyword`, `secondaryKeywords`, `h2Outline`, `aiPrompts`, `ctaRules`, `writingPreferences`, author and reviewer job title, description and credentials, every string in `facts`; the Content Maker 5.0 language map; batches of 50), `Translate Inputs` (DeepL, source = target code, target EN-US, five retries) and `Request EN` (English values into the body, originals under `_original`, DeepL target code under `_deeplTarget`). Parse Request exposes `original`, `deeplTarget`, `authorLocal` and `reviewerLocal`, and derives the slug from the original H1. Everything from Load Spec to Assemble Assets runs in English exactly as tested.
- **One assets object, four parts translated.** Collect FAQ and Assemble Meta are replaced by `Assemble Assets`, which reads Extract Updated, FAQ Writer (client and CTA sentences filtered), Metadata Generator (150-character clamp) and EEAT Analysis and outputs `finalPage`, `metaDescription`, `faq` and `eeat` as separate fields. DeepL Prep sends page, meta description, FAQ questions and answers, the four E-E-A-T evidence lines and the priority fix in one batch; Unwrap Translation puts the app's original H1 and H2 wording back over DeepL's rendering (the appended closing H2 stays translated). Build JSON-LD and Build Payload use `authorLocal` and `reviewerLocal`, and the payload's `eeat` is the translated one.
- **Clean Text before the schema and the callback (2026-09-22).** One Code node between Final Page and Build JSON-LD sanitises every shipped text field on every run and in every language: full HTML entity decoding (named, numeric, accented, double-escaped), invisible characters and BOM removed, CRLF and narrow no-break spaces normalised, markdown repaired after translation (link spacing and URL spaces, bold markers, `1)` step markers, heading spacing, table separator rows), em dashes and word-level en dashes turned into commas (number ranges kept), straight apostrophes aligned with the typographic ones the language uses, FAQ, meta and E-E-A-T lines stripped of bold, links and semicolons. It reports `textCleaned` in the item so a language with a new artefact shows up in the execution. The English page the evaluator scores is untouched.
- **FAQ required or not (2026-09-22).** `faq_required` (Y/N) in `page_type_specs`, set by `tools/extend_specs.py`: Y for the 72 types whose schema lists FAQPage or whose family is guide, evaluation, offer or local; N for the 56 hubs, reference, proof, entity, tool, asset, catalogue and ops types and for `faq_page`, whose questions the writer puts in the body as H3s (Build JSON-LD then builds FAQPage from those H3s). Page Contract turns it into `faqCount` 5 or 0; an `FAQ?` If node in front of FAQ Writer skips the model and a `No FAQ` node hands an empty list to Assets Merger; the callback then carries no `faq` key and no FAQPage block.
- **Search intent and the 16-column spec (2026-09-24).** `searchIntent` (informational, commercial, transactional, navigational) is required in the request; Parse Request rejects any other value. Prep Writer puts a page type line and an intent block at the top of the brief (what the reader wants first, which formats carry the sections, how the page ends) and states that both are enforced; the structure rules replace the per-type `format_rules` and `h3_policy` columns: the writer decides prose, steps, table or bullets per section from the heading and its research, puts at least two H3s under the H2 whose subject splits, keeps every supplied H2 word for word and may add H2s of its own instead of overloading a section; the facts are blended in where they support the point, never listed, and the CTA is one natural sentence at the end of the closing following `ctaRules`. Prep Meta builds the description rules from the intent (what it must contain, what leads it) inside the same prompt text. The spec table went from 31 to 16 columns (`words_count_approx` instead of the band, `definition` rewritten for all 128 types, `cta_mode` required or none, no fact lists, constraints, citation minimums, closing content, CTA guidance or meta patterns), generated by `tools/build_specs.py`; `Blocked?` no longer triggers, since no type is template-only and no facts are mandatory.
- **Monitoring prompts.** `Prompt Maker` (Gemini Latest, GPT 5.6 Luna fallback) runs in the assets branch with FAQ, meta and E-E-A-T and writes the unbranded H1 prompt and one branded prompt from the page. Assemble Assets builds `monitoringPrompts`: the input `aiPrompts` word for word in the page language (English rendering alongside), then the two generated ones, translated with the page on non-English runs and cleaned by Clean Text. Three to five per page, the budget Peec AI and Profound customers actually track.
- **Exact wording through translation (2026-09-22, afternoon).** Found on the Ukrainian attorney bio: the JSON-LD carried the English H1, the firm name came back from DeepL in two variants, and the writer had transliterated the name itself because `clientName` was the one input left untranslated. Now the client name is translated with the other inputs and every mention inside the description, facts and CTA rule is a protected token during input translation (Translate Inputs runs in HTML mode), so the writer sees one English form; on the way back each mention is wrapped in a `translate="no"` span holding the original, DeepL leaves it, Unwrap strips the span and collapses a duplicated generic noun DeepL sometimes adds in front. Clean Text rewrites the H1 and outline H2s from the request on every run (only when the H2 count still matches the outline; otherwise `textCleaned.headingsMismatch` is set). Build JSON-LD and the progress reports use the app's original H1, keyword and client name. The payload reports `coreKeywordMentions` and `clientNameMentions`.
- **Evaluator on the English page.** `Extract CM6 Article` in Money Calculator reads the `Extract Updated` output (the Style Updater's page) instead of `articleTextMd`, so the 5.0 metrics and rules score English text on every run.
...
| 25302 | `service_page`, English, same input as 15926, after the research chain was removed and the 5.0 writer guidelines ported | Success in 13 min 31 s, of which the writer took 9 min 17 s and made one Live Research call (a retained search fee source, cited in the page). Validation passed on the first pass, all 4 prompts judged answered without a fix round, 3 internal links placed once each, 8 H2s verbatim, 13 table rows, 5 FAQs, callback received. |
| 26289 | `service_page`, English, same input minus internal links and per-heading formats, after the 2026-09-15 revision | Success in 5 min 32 s. Nine H2s: the eight from the outline verbatim plus the appended closing section "How to get started with a sales leadership search" ending with the one CTA link. No links to the client site elsewhere. Two tables (timeline, specialist versus generalist), one deliverables list, one cited external figure. Six peelers on Sol flagged 33 sentences (11 series of three, 11 AI phrasing, 6 negative parallelisms, 3 tailing negations, 1 participle pile up, 1 hedge), all rewritten by the Style Updater. Validation failed once on semicolons and passed after the fix. All 4 prompts judged answered. FAQ: five neutral questions, no client name, no CTA. Meta description 154 characters, client name first, one concrete deliverable. |
| 36478 | `service_page`, English, same input plus `author`, after the 2026-09-21 revision | Success in 6 min 3 s. Seven inline progress calls fired (5, 15, 25, 35, 50, 88, 95 percent). Nine H2s with the appended closing section and one CTA link. FAQ, meta and EEAT ran in parallel and merged. Author Person in the payload. Validation reported one remaining issue (three semicolons left by the writer), which now stays on the page as a report only. Peelers on Sol flagged 39 sentences. The Money Calculator recorded USD 0.42 for the run and the restored 5.0 evaluator scored the page 63.4 (rules: 3 series of three, 5 hedges, 3 semicolons, 4 repeated arguments). |
| 36513 | `how_to`, English, no slugPath, no ctaRules, two facts, `author` | Success in 7 min 0 s. Slug derived from the H1, closing H2 "After you finish" with the one CTA link, three H3s in the outcomes section, one table, four external citations, callback body clean of progress fields. Validation reported semicolons only. The Money Calculator evaluator recorded the page in the EVALUATOR sheet as Flow Version "Content Maker 6.0" with quality score 69.0. The HowTo schema block was omitted because the page used H3s instead of a numbered step list, which is the builder's rule. |
...
| 38264 | `deep_dive_guide`, **French request** (every text value in French: description, H1, 7 H2s, 2 reader questions, CTA rule, preferences, author, 4 facts), Ferrari Classiche certification | Success in 6 min 56 s, first run with input translation. 22 French values to DeepL and back in English in 0.4 s; writer produced 2,094 English words, 8 H2s, 18 H3s, a table, 4 numbered steps, 5 citations. Final page 2,579 French words with the app's H1 and H2s restored verbatim, closing H2 translated ("Étapes suivantes"), French FAQ, E-E-A-T and meta description, CTA link once in the closing section. Evaluator (Money Calculator 38281) scored the English Style Updater page: quality 73.1. Two defects found: DeepL entities (`&#x27;`) left in the text and the meta description at 248 characters after translation; both fixed in Unwrap Translation. |
| 38286 | same request, run while the editor held a stale copy of Unwrap Translation | Success in 8 min 28 s; the defects above reproduced because the run used the stale node. Republished after reloading the repo code (version 3e219e05). |
| 38309 | same request on the published version | Success in 6 min 58 s. 2,468 French words, the app's H1 and 7 H2s verbatim plus "Étapes suivantes", 20 H3s, a table, 4 numbered steps, 5 citations, 1 CTA link, meta description 112 characters, 5 French FAQs, French E-E-A-T (6.0). No entities, semicolons or dashes left in any field. |
| 38355 | `industry_vertical_page`, EPAM, financial services, English | Success in 5 min 23 s. 1,201 words, 6 H2s incl. appended "How to get started in financial services", a pains table, 4 citations (Oliver Wyman, Swift, EIOPA, Evident), one CTA link, meta 148 characters, 5 FAQs none naming EPAM, Service + FAQPage + BreadcrumbList, E-E-A-T 7.0. |
| 38359 | `service_page`, EPAM, cloud migration, English | Success in 5 min 22 s. 974 words, 6 H2s, three tables (phases, approaches, pricing), one bullet checklist, 1 citation, one CTA link, meta 144, 5 FAQs, Service + Organization + FAQPage + BreadcrumbList, E-E-A-T 6.5. One semicolon left in the body by the writer and Style Updater. |
| 38363 | `glossary_term`, EPAM, landing zone, English | Success in 2 min 10 s. 416 words, 4 H2s as sent, no closing, no CTA, no H3, related terms as bullets, DefinedTerm + DefinedTermSet + BreadcrumbList, meta 148 without the client name, E-E-A-T 6.3. No citation although the spec asks for one (research is off for this type). Five FAQs were still written for a type that carries no FAQPage. |
| 38367 | `author_bio_page`, EPAM author, English | Success in 5 min 26 s. 1,222 words, 4 H2s as sent, 3 H3s, credentials and articles as bullets, no table, no CTA, Person + ProfilePage + BreadcrumbList with the author from the request, meta 147, E-E-A-T 6.3. Five FAQs written for a page type that has none. |
| 38378 | `practice_area_page`, **Ukrainian request**, Lviv law firm, family law | Success in 7 min 25 s. 22 Ukrainian values translated to English (DeepL detected UK), 2,062 English words written, 1,985 Ukrainian words returned with the app's H1 and 6 H2s verbatim, translated closing H2, 8 H3s, a table, 7 numbered steps, one CTA link, meta 144, 5 Ukrainian FAQs none naming the firm, Ukrainian E-E-A-T, LegalService + FAQPage + Person + BreadcrumbList, `inLanguage: uk`. Zero entities, semicolons or dashes. No external citation (the spec sets none for this type) although `zakon.rada.gov.ua` was whitelisted. |
| 38481 | `glossary_term`, EPAM, English, after the FAQ switch | Success in 2 min 31 s. FAQ? routed to No FAQ, FAQ Writer never ran, the callback has no `faq` key and no FAQPage block. `monitoringPrompts`: the input prompt word for word, one H1 prompt, one branded prompt. |
| 38485 | `service_page`, EPAM, English, after the FAQ switch | Success in 5 min 1 s. FAQ Writer ran, five FAQs, FAQPage block present, four monitoring prompts (two input, H1, branded). |
| 38490 | `deep_dive_guide`, French request, after the FAQ switch | Success in 7 min 47 s. Five French FAQs, and the monitoring prompts in French with their English rendering: the two input prompts word for word, the H1 prompt and the branded prompt translated with the page. |
| 38540 | `attorney_bio`, **Ukrainian request**, Lviv law firm, a type with no FAQ | Success in 2 min 37 s. 801 Ukrainian words, the app's H1 and 4 H2s verbatim plus a translated closing H2 with the CTA link once, credentials as bullets, no table, no `faq` key, no FAQPage; ProfilePage + Person + BreadcrumbList with the Ukrainian author; three monitoring prompts (input word for word, H1 and branded in Ukrainian). Found: the JSON-LD `name` and `headline` carry the English H1 instead of the app's original (Build JSON-LD reads `r.h1`, should read `r.original.h1`), the branded `promptEn` keeps the Ukrainian firm name because client names are never translated, and DeepL renders the firm name in two variants in the body («Галицький та партнери», «Галицький і партнери»). |
| 38620 | `attorney_bio`, Ukrainian, after the wording fixes (first pass) | Success in 2 min 45 s. JSON-LD name in Ukrainian, headings verbatim, no `faq` key. The firm name still drifted («Галицький та партнери») because DeepL had rendered `clientName` and the description differently and the writer used the description's spelling; the branded prompt read «юридична фірма «Юридична фірма …»». Both fixed in the second pass (token in the input translation, generic-noun collapse). |
| 38624 | `glossary_term`, EPAM, English, after the wording fixes | Success in 1 min 56 s. Headings verbatim, no `faq` key, DefinedTerm term code from the request, keyword mentions 10. |
| 38629 | `deep_dive_guide`, French, after the wording fixes | Success in 7 min 18 s. H1 and 7 H2s verbatim, closing "Que faire ensuite ?", JSON-LD name in French, keyword mentions 5, client mentions 38, French FAQs and prompts. |
| 38667 | `attorney_bio`, Ukrainian, after the client-name token in the input translation | Success in 3 min 33 s. The writer used one English form of the firm name; the Ukrainian page, closing CTA sentence and branded prompt carry «Юридична фірма Галицький та Партнери» exactly (client mentions 2, no variant), JSON-LD name in Ukrainian, headings verbatim, no `faq` key. Still seen: DeepL puts an inflected generic noun in front of the protected name («юридичній фірмі «Юридична фірма …»»), collapsed since by a stem match in Unwrap Translation (verified on this run's DeepL output locally); the core keyword appears only inflected, reported as `coreKeywordMentions: 0`. |
| 42124 | `glossary_term`, EPAM, data mesh, English, **`searchIntent: informational`**, after the 16-column spec and the intent-driven brief | Success in 2 min 17 s. 537 words against a 500-word target, the 3 H2s word for word, 4 H3s under the H2 that splits (the four principles), one comparison table, a checklist, 2 citations, no CTA, no `faq` key, meta description 144 characters without the client name (informational rule). |
| 42120 | `service_page`, EPAM, cloud migration, English, **`searchIntent: transactional`** | Success in 5 min 40 s. 1,430 words against 1,500, the offer stated in the opening paragraph, 5 H2s word for word plus the appended closing "How to get started with cloud migration services", 2 H3s under the approaches section, 2 tables (7 Rs, cost factors), a 4-step list, the deliverables as a bullet list, 4 citations (AWS, McKinsey, IBM), one CTA link as the last sentence, meta 131 characters led by the client name (transactional rule), 5 FAQs, E-E-A-T 8.8. |
| 42128 | `practice_area_page`, **Ukrainian request**, Kyiv family law firm, **`searchIntent: transactional`** | Success in 7 min 14 s. 1,591 Ukrainian words against 2,000, 5 H2s plus a closing, 6 H3s, 2 tables, 2 step lists, 7 citations (Family Code articles, 2026 fee figures), one CTA link, 5 FAQs, E-E-A-T 8.3, meta led by the firm name. Two defects: DeepL returned the protected firm name as a paragraph of its own between « and » («Юридична фірма «\n\nЮридична фірма Правова Опора\n\n»»), and copied that pattern into the one outline H2 that carried a `translate="no"` span. Fixed: headings are no longer wrapped for DeepL but mapped by position in DeepL Prep (`headingMap`, so H2s the writer adds keep their translated heading) and restored in Unwrap Translation; a span DeepL broke out into its own paragraph is reattached inline, and the quotes and inflected generic noun around the name collapse to the app's exact name (replayed on this run's DeepL output: 4 exact mentions, no stray quotes, every heading verbatim). |
| 42167 | `practice_area_page`, Ukrainian, same request, after the translation fix (version 877628ef, published) | Success in 7 min 6 s. 1,805 Ukrainian words, the app's H1 and 5 H2s word for word plus the closing «Зверніться до адвоката з сімейних справ» (mapped by position, `headingsRestored: true`, `headingsMissing: []`), 5 H3s, a table, one CTA link, 5 FAQs, meta 117 characters led by the firm name, the firm name exactly as sent 4 times with no quotes, no blank-line paragraphs, no entities and no semicolons. |
